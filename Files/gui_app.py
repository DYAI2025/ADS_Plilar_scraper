...
class ADSPillarGUI:
    def __init__(self, root):
        ...
        if not MODULES_AVAILABLE or not NICHE_AVAILABLE:
            error_msg = (
                "Some modules could not be loaded due to missing dependencies:\n\n"
            )
            if IMPORT_ERROR_MSG:
    ...
    def setup_gui(self):
        ...
        self.status_bar = ttk.Label(
            self.root, text="Bereit", relief=tk.SUNKEN, anchor=tk.W
        )
    ...
    def create_setup_tab(self):
        ...
        # Config fields
        ttk.Label(config_frame, text="Website Name:").grid(
            row=0, column=0, sticky="w", padx=5, pady=5
        )
        ttk.Entry(
            config_frame, textvariable=self.project_config["site_name"], width=40
        ).grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(config_frame, text="Domain:").grid(
            row=1, column=0, sticky="w", padx=5, pady=5
        )
        ttk.Entry(
            config_frame, textvariable=self.project_config["domain"], width=40
        ).grid(row=1, column=1, padx=5, pady=5)

        ttk.Label(config_frame, text="Stadt:").grid(
            row=2, column=0, sticky="w", padx=5, pady=5
        )
        ttk.Entry(
            config_frame, textvariable=self.project_config["city"], width=40
        ).grid(row=2, column=1, padx=5, pady=5)

        ttk.Label(config_frame, text="Kategorie:").grid(
            row=3, column=0, sticky="w", padx=5, pady=5
        )
        ttk.Entry(
            config_frame, textvariable=self.project_config["category"], width=40
        ).grid(row=3, column=1, padx=5, pady=5)

        ttk.Label(config_frame, text="AdSense ID:").grid(
            row=4, column=0, sticky="w", padx=5, pady=5
        )
        ttk.Entry(
            config_frame, textvariable=self.project_config["adsense_id"], width=40
        ).grid(row=4, column=1, padx=5, pady=5)

        ttk.Label(config_frame, text="Google Analytics ID:").grid(
            row=5, column=0, sticky="w", padx=5, pady=5
        )
        ttk.Entry(
            config_frame, textvariable=self.project_config["ga_id"], width=40
        ...
        ttk.Button(
            button_frame,
            text="🚀 Vollständiges Setup starten",
            command=self.run_full_setup,
            style="Accent.TButton",
        ).pack(side="left", padx=(0, 10))
        ttk.Button(
            button_frame, text="💾 Konfiguration speichern", command=self.save_config
        ).pack(side="left", padx=(0, 10))
        ttk.Button(
            button_frame, text="📁 Konfiguration laden", command=self.load_config
        ).pack(side="left")
    ...
    def create_niche_tab(self):
        ...
        ttk.Button(
            control_frame, text="🔍 Nischen analysieren", command=self.analyze_niches
        ).pack(side="left", padx=(0, 10))
        ttk.Button(
            control_frame, text="📊 Keyword Research", command=self.keyword_research
        ).pack(side="left", padx=(0, 10))
        ttk.Button(
            control_frame, text="💡 Review Demand Analyse", command=self.analyze_demand
        ).pack(side="left")

        # Results table
        columns = ("Nische", "Score", "Suchvolumen", "Competition", "RPM Potenzial")
        self.niche_tree = ttk.Treeview(
            main_frame, columns=columns, show="tree headings", height=10
        )
    ...
    def create_data_tab(self):
        ...
        ttk.Checkbutton(
            ...
            variable=self.data_sources["google_places"],
        ).pack(anchor="w", padx=5, pady=2)
        ttk.Checkbutton(
            source_frame, text="Manuelle CSV", variable=self.data_sources["manual_csv"]
        ).pack(anchor="w", padx=5, pady=2)
        ttk.Checkbutton(
            source_frame,
            text="Yelp API (Coming Soon)",
            variable=self.data_sources["yelp"],
            state="disabled",
        ).pack(anchor="w", padx=5, pady=2)
        ttk.Checkbutton(
            source_frame,
            text="Foursquare API (Coming Soon)",
            variable=self.data_sources["foursquare"],
            state="disabled",
        ).pack(anchor="w", padx=5, pady=2)
        ...
        ttk.Label(api_frame, text="Google Places API Key:").grid(
            row=0, column=0, sticky="w", padx=5, pady=5
        )
        self.google_api_key = tk.StringVar()
        ttk.Entry(api_frame, textvariable=self.google_api_key, width=50, show="*").grid(
            row=0, column=1, padx=5, pady=5
        )

        ttk.Label(api_frame, text="Search Query:").grid(
            row=1, column=0, sticky="w", padx=5, pady=5
        )
        self.search_query = tk.StringVar(value="parks")
        ttk.Entry(api_frame, textvariable=self.search_query, width=50).grid(
        ...
        ttk.Button(
            control_frame, text="🔄 Daten sammeln", command=self.collect_data
        ).pack(side="left", padx=(0, 10))
        ttk.Button(control_frame, text="📁 CSV laden", command=self.load_csv).pack(
            side="left", padx=(0, 10)
        )
        ttk.Button(control_frame, text="💾 CSV speichern", command=self.save_csv).pack(
            side="left"
        )
    ...
    def create_generate_tab(self):
        ...
        ttk.Radiobutton(
            template_frame,
            text="Standard Pillar Page",
            variable=self.template_var,
            value="pillar_page_skeleton.html",
        ).pack(anchor="w", padx=5, pady=2)
        ttk.Radiobutton(
            template_frame,
            text="Mobile-First Template",
            variable=self.template_var,
            value="mobile_template.html",
        ).pack(anchor="w", padx=5, pady=2)
        ttk.Radiobutton(
            template_frame,
            text="High-RPM Template",
            variable=self.template_var,
            value="rpm_optimized.html",
        ).pack(anchor="w", padx=5, pady=2)
        ...
        for key, var in self.gen_options.items():
            ...
            ttk.Checkbutton(options_frame, text=text, variable=var).grid(
                row=row // 2, column=row % 2, sticky="w", padx=5, pady=2
            )
            row += 1
        ...
        ttk.Button(
            control_frame, text="🏗️ Seite generieren", command=self.generate_page
        ).pack(side="left", padx=(0, 10))
        ttk.Button(
            control_frame, text="👀 Vorschau öffnen", command=self.preview_page
        ).pack(side="left", padx=(0, 10))
        ttk.Button(
            control_frame, text="📤 Upload zu Server", command=self.upload_page
        ).pack(side="left")
    ...
    def create_revenue_tab(self):
        ...
        for i, (label, value, trend) in enumerate(kpis):
            ...
            ttk.Label(kpi_frame, text=label).grid(
                row=row_offset + i // 2, column=(i % 2) * 3, sticky="w", padx=5, pady=5
            )
            ttk.Label(
                kpi_frame, text=value, font=("Arial", 12), foreground="gray"
            ).grid(row=row_offset + i // 2, column=(i % 2) * 3 + 1, padx=5, pady=5)
            ttk.Label(kpi_frame, text=trend, foreground="gray").grid(
                row=row_offset + i // 2, column=(i % 2) * 3 + 2, padx=5, pady=5
            )
        ...
        ttk.Label(calc_frame, text="Monatliche Pageviews:").grid(
            row=0, column=0, sticky="w", padx=5, pady=5
        )
        ...
        ttk.Label(calc_frame, text="Page RPM (€):").grid(
            row=1, column=0, sticky="w", padx=5, pady=5
        )
        ...
        self.revenue_label = ttk.Label(
            calc_frame,
            text="Geschätzter Umsatz: €0/Tag (≈ €0/Monat)",
            font=("Arial", 12, "bold"),
        )
        ...
        ttk.Button(
            analytics_frame,
            text="🔗 Mit Google Analytics verbinden",
            command=self.connect_analytics,
        ).pack(pady=10)
        ttk.Button(
            analytics_frame,
            text="💰 AdSense Dashboard öffnen",
            command=self.open_adsense,
        ).pack(pady=5)

        self.analytics_status = ttk.Label(
            analytics_frame, text="Status: Nicht verbunden"
        )
    ...
    def run_full_setup(self):
        ...
        def setup_thread():
            try:
                ...
                # Save configuration
                config_data = {
                    key: var.get() for key, var in self.project_config.items()
                }
                ...
                for rec in recommendations[:3]:
                    self.log_message(
                        f"   • {rec['niche']['name']} (Score: {rec['opportunity_score']})"
                    )
        ...
        def collect_thread():
            try:
                ...
                if places:
                    ...
                    messagebox.showinfo(
                        "Erfolg", f"{len(places)} Orte erfolgreich gesammelt!"
                    )
                else:
                    messagebox.showwarning(
                        "Keine Daten",
                        "Keine Daten gefunden. API Key oder Query prüfen.",
                    )
                    self.update_status("Bereit")
    ...
    def generate_page(self):
        ...
        def generate_thread():
            try:
                ...
                self.log_message(
                    f"✅ {len(df)} Locations geladen ({data_source})", self.gen_log
                )
                ...
                for _, row in df.iterrows():
                    loc = LocationData(
                        ...
                        feature_wheelchair_accessible=bool(
                            row.get("feature_wheelchair_accessible", False)
                        ),
                        feature_kids_friendly=bool(
                            row.get("feature_kids_friendly", False)
                        ),
                        feature_dogs_allowed=bool(
                            row.get("feature_dogs_allowed", False)
                        ),
                ...
                try:
                    ...
                    self.log_message(
                        f"⚠️ ID-Injektion fehlgeschlagen: {_e}", self.gen_log
                    )
                ...
                messagebox.showinfo(
                    "Erfolg", f"Seite erfolgreich generiert!\n{output_path}"
                )
                ...
                messagebox.showerror(
                    "Fehler", f"Fehler bei Seiten-Generierung: {str(e)}"
                )
    ...
    def preview_page(self):
        ...
        if os.path.exists(output_path):
            ...
            messagebox.showwarning(
                "Datei nicht gefunden", "Bitte zuerst eine Seite generieren."
            )
    ...
    def save_csv(self):
        ...
        try:
            if not self.data_preview.get_children():
                messagebox.showwarning(
                    "Keine Daten", "Keine Daten zum Exportieren vorhanden."
                )
                return
        ...
        def research_thread():
            try:
                ...
                self.niche_details.insert(
                    tk.END, f"📊 Keyword Analyse für: {category} in {city}\n"
                )
                ...
                self.niche_details.insert(
                    tk.END, f"✅ Niedrige Competition ({len(low_comp)} Keywords):\n"
                )
                ...
                self.niche_details.insert(
                    tk.END, f"💡 Empfehlung: Fokus auf Low-Competition Keywords\n"
                )
    ...
    def analyze_demand(self):
        ...
        def demand_thread():
            try:
                ...
                self.niche_details.delete("1.0", tk.END)
                self.niche_details.insert(
                    tk.END, "🔍 Review Demand Analyse läuft...\n\n"
                )
                ...
                if len(api_key) < 20 or not api_key.startswith("AIza"):
                    response = messagebox.askyesno(
                        "API Key Warnung",
                        "Der API-Key scheint ungültig zu sein (zu kurz oder falsches Format).\n\n"
                        "Google Places API-Keys beginnen normalerweise mit 'AIza' und sind mindestens 20 Zeichen lang.\n\n"
                        "Möchten Sie trotzdem fortfahren?",
                    )
                    if not response:
                        self.niche_details.insert(
                            tk.END, "❌ Abgebrochen: Ungültiger API Key\n"
                        )
                        self.update_status("Bereit")
                        return

                self.niche_details.insert(
                    tk.END, f"📍 Analysiere: {category} in {city}\n"
                )
                self.niche_details.insert(tk.END, "=" * 60 + "\n\n")
                ...
                if analysis["total_reviews_analyzed"] == 0:
                    ...
                    self.niche_details.insert(
                        tk.END, "   Tipp: Prüfe API Key und Kategorie/Stadt\n"
                    )
                    self.update_status("Bereit")
                    return
                ...
                self.niche_details.insert(
                    tk.END,
                    f"   Reviews analysiert: {analysis['total_reviews_analyzed']}\n",
                )
                self.niche_details.insert(
                    tk.END,
                    f"   Durchschnittliche Bewertung: {analysis['avg_rating']:.2f}/5.0\n",
                )
                self.niche_details.insert(
                    tk.END, f"   Sentiment Score: {analysis['sentiment_score']:.2f}\n\n"
                )
                ...
                for i, (phrase, count) in enumerate(analysis["top_complaints"][:7], 1):
                    self.niche_details.insert(
                        tk.END, f"   {i}. '{phrase}' ({count}x)\n"
                    )
                self.niche_details.insert(tk.END, "\n")

                # Unmet needs
                self.niche_details.insert(
                    tk.END, "💡 UNERFÜLLTE BEDÜRFNISSE (Opportunities!):\n"
                )
                if analysis["unmet_needs"]:
                    for i, (feature, count) in enumerate(
                        analysis["unmet_needs"][:5], 1
                    ):
                        self.niche_details.insert(
                            tk.END,
                            f"   {i}. {feature.upper()} ({count} Erwähnungen) ⭐\n",
                        )
                else:
                    self.niche_details.insert(
                        tk.END, "   Keine erkannt - alle Bedürfnisse gedeckt\n"
                    )
                self.niche_details.insert(tk.END, "\n")
                ...
                self.niche_details.insert(
                    tk.END, "🎯 CONTENT-IDEEN (Sofort umsetzbar!):\n"
                )
                self.niche_details.insert(tk.END, f"{'='*60}\n\n")

                for i, idea in enumerate(ideas[:4], 1):  # Show top 4 ideas
                    self.niche_details.insert(
                        tk.END, f"{i}. [{idea['priority']}] {idea['title']}\n"
                    )
                    self.niche_details.insert(tk.END, f"   Typ: {idea['type']}\n")
                    self.niche_details.insert(
                        tk.END, f"   Impact: {idea['estimated_impact']}\n"
                    )
                    self.niche_details.insert(
                        tk.END, f"   Beschreibung: {idea['description']}\n\n"
                    )
    ...
    def _prompt_api_key(self):
        ...
        ttk.Label(
            dialog, text="Google Places API Key benötigt:", font=("Arial", 12, "bold")
        ).pack(pady=(20, 10))
        ...
        ttk.Button(button_frame, text="Abbrechen", command=on_cancel).pack(
            side="left", padx=5
        )
    ...
    def create_location_page_tab(self):
        ...
        ttk.Label(form, text="Titel/Ort:").grid(
            row=0, column=0, sticky="w", padx=6, pady=6
        )
        ttk.Entry(form, textvariable=self.location_form["title"], width=40).grid(
            row=0, column=1, padx=6, pady=6
        )

        ttk.Label(form, text="Unterzeile/Tagline:").grid(
            row=1, column=0, sticky="w", padx=6, pady=6
        )
        ttk.Entry(form, textvariable=self.location_form["tagline"], width=60).grid(
        ...
        # Image folder
        ttk.Label(paths, text="Bilder-Ordner:").grid(
            row=0, column=0, sticky="w", padx=6, pady=6
        )
        ttk.Entry(
            paths, textvariable=self.location_form["image_folder"], width=60
        ).grid(row=0, column=1, padx=6, pady=6)
        ttk.Button(paths, text="Auswählen…", command=self._browse_images).grid(
            row=0, column=2, padx=6, pady=6
        )

        # Output dir
        ttk.Label(paths, text="Ausgabe-Verzeichnis:").grid(
            row=1, column=0, sticky="w", padx=6, pady=6
        )
        ttk.Entry(paths, textvariable=self.location_form["output_dir"], width=60).grid(
            row=1, column=1, padx=6, pady=6
        )
        ttk.Button(paths, text="Auswählen…", command=self._browse_output).grid(
        ...
        ttk.Button(
            actions, text="Seite generieren", command=self._generate_location_page
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            actions, text="Vorschau öffnen", command=self._preview_location_page
        ).pack(side=tk.LEFT, padx=5)

        # Info
        self.location_info = ttk.Label(
            frame, text="Wähle einen Bilder-Ordner mit JPG/PNG/WebP-Dateien."
        )
