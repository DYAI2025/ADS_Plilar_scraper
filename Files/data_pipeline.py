...
class DataEnrichment:
    ...
    FEATURE_KEYWORDS = {
        'shade': ['schatten', 'schattig', 'bäume', 'überdacht', 'shadow', 'shaded', 'trees'],
        'benches': ['bank', 'bänke', 'sitzbank', 'sitzen', 'bench', 'benches', 'seating'],
        'water': ['wasser', 'brunnen', 'teich', 'see', 'bach', 'water', 'fountain', 'pond'],
        'parking': ['parkplatz', 'parken', 'parking', 'stellplatz', 'garage'],
        'toilets': ['toilette', 'wc', 'klo', 'sanitär', 'toilet', 'restroom', 'bathroom'],
        'wheelchair': ['rollstuhl', 'barriere', 'wheelchair', 'accessible', 'handicap', 'disabled'],
        'kids': ['kinder', 'spielplatz', 'familie', 'kids', 'children', 'family', 'playground'],
        'dogs': ['hund', 'hunde', 'dog', 'dogs', 'pet', 'tier', 'erlaubt'],
        'fee': ['kostenlos', 'gratis', 'free', 'frei', 'umsonst', 'ohne kosten'],
        'seasonal': ['saisonal', 'winter', 'sommer', 'geschlossen', 'seasonal', 'closed']
    }
    ...
    def extract_features_from_text(cls, text: str) -> Dict[str, bool]:
        ...
        for feature, keywords in cls.FEATURE_KEYWORDS.items():
            features[f'feature_{feature}'] = any(keyword in text_lower for keyword in keywords)
...
class DataScraper:
    ...
    def scrape_google_places(self, query: str, location: str, api_key: str) -> List[Dict]:
        """
    ...
    def get_place_details(self, place_id: str, api_key: str) -> Dict:
        ...
        try:
            ...
            reviews_text = " ".join([review.get('text', '') for review in result.get('reviews', [])])
            
            return {
                'phone': result.get('international_phone_number', ''),
                'website': result.get('website', ''),
                'opening_hours': str(result.get('opening_hours', {}).get('weekday_text', [])),
...
class PillarPageGenerator:
    ...
    def generate_page(self, data: List[LocationData], city: str, category: str, 
                     output_path: str, canonical_url: str) -> None:
        ...
        for i, location in enumerate(data):
            ...
            schema_items.append({
                ...
                "item": {
                    ...
                    } if location.rating > 0 else None
                }
            })
