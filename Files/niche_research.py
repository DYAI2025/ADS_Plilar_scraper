...
class KeywordResearch:
    ...
    def check_serp_diversity(self, keyword: str) -> Dict:
        ...
        mock_serp = {
            ...
            'top_domains': ['yelp.com', 'tripadvisor.com', 'foursquare.com', 'local-site.de'],
    ...
    def generate_keyword_variations(self, base_keyword: str, cities: List[str]) -> List[Dict]:
...
class NicheValidator:
    ...
    NICHE_IDEAS = [
        {
            ...
            'facets': ['zaun', 'wasser', 'schatten', 'größe', 'agility', 'kleine_hunde'],
        ...
        },
        {
            ...
            'keywords': ['coworking', 'wifi cafe', 'arbeitsplatz', 'laptop cafe', 'study spot'],
        ...
        {
            'name': 'Spielplätze',
            'keywords': ['spielplatz', 'playground', 'kinderpark', 'abenteuerspielplatz'],
        ...
        {
            'name': 'Fotospots',
            'keywords': ['fotospot', 'instagram spot', 'photo location', 'aussichtspunkt'],
            'facets': ['beste_zeit', 'equipment', 'erlaubnis', 'menschenmenge', 'wetter'],
    ...
    def analyze_niche(self, niche_name: str) -> Dict:
        ...
        niche = next((n for n in self.NICHE_IDEAS if n['name'].lower() == niche_name.lower()), None)
        if not niche:
        ...
        for keyword in niche['keywords'][:2]:  # Limit for demo
            variations = researcher.generate_keyword_variations(keyword, niche['cities'][:3])
            all_keywords.extend(variations)
        
        # Calculate opportunity score
        total_volume = sum(kw['estimated_volume'] for kw in all_keywords)
        low_competition_count = sum(1 for kw in all_keywords if kw['competition'] == 'Low')
        
        opportunity_score = min(100, (total_volume / 1000) + (low_competition_count * 5))
...
def main():
    ...
    if recommendations:
        ...
        for kw in sample_keywords:
            print(f"   • {kw['keyword']} (Vol: {kw['estimated_volume']}, Comp: {kw['competition']})")
