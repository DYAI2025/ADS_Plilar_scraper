...
<script>
  ...
  (adsbygoogle = window.adsbygoogle || []).push({{
    ...
    def generate_schema_markup(self, location_data: List[Dict], city: str, category: str) -> str:
...
class ProjectSetup:
    ...
    def generate_config_files(self, output_dir: str = "."):
        ...
        if self.config.get('adsense_id'):
            ads_content = f"google.com, {self.config['adsense_id']}, DIRECT, f08c47fec0942fa0\n"
        ...
        sample_pages = [
            ...
            {'path': f"/{self.config.get('main_category', 'places')}/", 'priority': 0.9},
